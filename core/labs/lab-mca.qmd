...
Do GSS surveys are made of a huge number of questions. Not all questions were asked to all respondents. Indeed, each question was asked to two thirds of the respondents. Questions were related to demographic features (Age, Sex, Level of education, Employment and so on). Answers to these questions can be used to determine whether the panel sample is representative. Data can be compared with last census data (there is a census every ten years in the US). 
...
```{r}
#| label: confidence_ids_wave_1bis

cleanup_text <- function(str_col) {
    {str_split(str_col, '\\? [a-z]\\. ', simplify = T)[,2]} |> 
    str_squish()
}
...
gss_panel_doc |>
  filter(id %in% confidence_ids) |> 
  select(-ends_with("2"), -ends_with("3")) |>
  mutate(text=cleanup_text(text)) |> 
  select(id, text) |> 
  gt::gt() |>
  gt::tab_caption("Question {id} consists of 'As far as the people running these institutions are concerned, would you say you have a great deal of confidence, only some confidence, or hardly any confidence at all in them? ' {text}")
...
confidence_panel10 <- gss_panel10_long |> 
  filter(wave==1) |>
  select(firstid, wave, id, sex, age, contains(confidence_ids)) |>
  mutate(across(starts_with("con"), as_factor)) |>
  mutate(sex=as_factor(sex))
```

```{r}
#| label: confidence_10_setup
...
levels_table <- confidence_10 |> 
  ...
  rename(encoding=`value...9`)

levels_table |> 
  gt::gt() |> 
  gt::tab_caption("Confidence levels encodings") |>
  gt::tab_options(
...
Use `skimr` to summarize your dataset. 
...
# cleanup_column_labels <- function(col_labs){
#   {str_split(col_labs, ':', simplify = T)[,2]} |> 
#     str_squish()
# }

confidence_data |> 
#  rename_with(.fn=, .cols=everything()) |> 
  slice_sample(n=10) |> 
  gt::gt()
```
...
$$\frac{\mathbb{I}_{i, \langle j, k\rangle}- f_{\langle j,k\rangle}}{\sqrt{N f_{\langle j,k\rangle}}}$$
   
   
The squared Hilbert-Schmidt (Frobenius) norm of matrix $S$ is 

$$\sum_{j\leq p} \sum_{k\leq J_j} \sum_{i\leq n} \frac{(\mathbb{I}_{i, \langle j, k\rangle}- f_{\langle j,k\rangle})^2}{N f_{\langle j,k\rangle}} = \sum_{j\leq p} \sum_{k\leq J_j}  \frac{n f_{\langle j,k\rangle}(1- f_{\langle j,k\rangle})}{N f_{\langle j,k\rangle}}=    \frac{\sum_{j\leq p} J_j- p}{p}$$
This is often called the total inertial of the standardized residuals matrix $S$.
...
> The principal coordinates are the principal coordinates of each individual in terms of the principal component.  These are the coefficients of $D_r^{-1/2} \times U \times D$ 
where $U, D$ come from the SVD  of the standardized residuals matrix $S$.

The columns of `coord` are pairwise orthogonal in the usual  inner product space (they are rescaled versions of the columns of $U$).

Function `augment_mca` binds the columns of `coord` with the columns of the original dataset.  
...
#  scale_color_manual(values=neat_color_scale) +
  geom_point(aes(fill=age), alpha=.8, size=.7) +
  coord_fixed()  
...
::: {.content-visible unless-profile="TOUT"}
...
c <- res.mca$call$marge.col

# Standardized residual matrix
S <- sqrt(n)*(as.matrix(P)- (matrix(1/n, nrow=n) %*% c)) %*% diag(c^(-1/2))

# with(res.mca$svd, norm(S-U %*% diag(vs[1:res.mca$call$ncp]) %*% t(V), type="F")^2)
U <- res.mca$svd$U
V <- res.mca$svd$V
vs <- res.mca$svd$vs

# $U$ and $V$ are rescaled components of svd(S)
norm(diag(res.mca$call$marge.row[1:5]) %*% t(U) %*% U - diag(rep(1,5)), type="F")
norm(diag(rep(1,5))- t(V) %*% diag((res.mca$call$marge.col)) %*% V, type="F")


Vp <- diag((res.mca$call$marge.col)^(1/2)) %*% V
Up <- diag((res.mca$call$marge.row)^(1/2)) %*% U

norm(S - Up %*% diag(vs[1:5]) %*% t(Vp), type="F")^2 - sum(vs[6:39]^2)
```
...
tidied_mca_var <- tidy_mca_var(res.mca) |>   
  ...
  mutate(topic=map_chr(sentiment, \(x)x[1]), 
         sentiment=map_chr(sentiment, \(x)x[2])
  ) 
...
The $v$-test statistic is computed as:
$$v = \frac{p - P}{\sqrt{P(1 - P)/n}}$$
where:
- $p$ is the proportion of individuals in a given category for the specific dimension,
- $P$ is the  overall proportion of individuals in that category across all dimensions,
